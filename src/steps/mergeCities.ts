 ...
 * fills part of the distance). A single pass over the plain-merged cities:
...
function mergeTriangles(ctx: PipelineContext, settings: Settings, cities: City[]): City[] {
  const local = cities.map((c) => c.localPolygon);
  const bboxes: BBox[] = local.map((p) => bboxOf(allPositions(p.geometry)));
  const centroids = local.map((p) => {
    const h = convexHull(allPositions(p.geometry));
  ...
  const spanOk = (a: number, c: number, b: number): boolean => {
    ...
    if (ok === undefined) {
      ...
      ok = bboxGap(bboxes[a], bboxes[c]) <= limit && polygonGapUnder(local[a], local[c], limit);
...
/** Combine each union-find group into a single city. */
